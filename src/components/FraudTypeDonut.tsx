...
// Parse the CSV data for fraud types
const fraudTypeData = [
  { name: 'Spoofed', value: 26, color: '#e74c3c' },
  { name: 'Robocall', value: 6, color: '#f39c12' },
  { name: 'Fraud/Scam Risk', value: 18, color: '#27ae60' }
];

const FraudTypeDonut: React.FC = () => {
  const [selectedFilter, setSelectedFilter] = useState('All');
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);

  const filterOptions = ['All', 'Spoofed', 'Robocall', 'Fraud/Scam Risk'];
  ...
  const getFilteredData = () => {
    ...
    // When a specific type is selected, highlight it
    return fraudTypeData.map(item => ({
      ...item,
  ...
  const getTotalValue = () => {
    ...
    const selectedItem = fraudTypeData.find(item => item.name === selectedFilter);
  ...
  const getSelectedPercentage = () => {
    ...
    const selectedItem = fraudTypeData.find(item => item.name === selectedFilter);
    const total = fraudTypeData.reduce((sum, item) => sum + item.value, 0);
